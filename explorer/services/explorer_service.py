...
class ExplorerService:
    ...
    def handle_universal_raw_operation(self, user_id, operation_name, operation_params):
        ...
        operation_map = {
            ...
            "undo_operation" : self.explorer_cache_service.get_most_recent_universal_raw_data_obj,   #Change below to cache along the way and only return last dataset
            "load_snapshot" : self.assemble_dataset_list_from_operation_chain
        } 
        if operation_params["operation_name"] in ["init", "filter", "traverse"]:
            ...
            self.explorer_cache_service.delete_most_recent_universal_data_and_operation_obj(user_id=user_id) #combine method for deleting both at once i think
            result_data, data_amount = operation_map[operation_name](**operation_params)
    ...
    def handle_explorer_state_operation(self, user_id, operation_name, operation_params):
        ...
        operation_obj = {"operation_name": operation_name, "operation_type": result_data_type, "operation_params":operation_params}
        operation_map = {
            "end_explorer_session" : self.explorer_cache_service.empty_explorer_cache, 
    ...
    def handle_snapshot_operation(self, user_id, operation_name, operation_params):
        debug_print()
        result_data_type = "snapshot" if operation_name is not "get_all_snapshots" else "snapshot_list"
        operation_obj = {"operation_name": operation_name, "operation_type": result_data_type, "operation_params":operation_params}
        operation_map = {
