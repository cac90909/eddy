...
class ExplorerService:
    ...
    def handle_operation(self, user_id, operation_name, operation_arguments):
        op_def = operation_util.get_operation_definition(operation_name=operation_name)
        #Request Validation
        if op_def is None:
        ...
        for arg in op_def.get("operation_arguments"):
            if arg.get("argument_name") not in operation_arguments and arg.get("argument_name") != "user_id" and arg.get("required"): #Suspending check on user_id for right now
                return OperationResult(data=None, meta={"data_type":"error", "error_message": f"Missing argument {arg.get("argument_name")}"}).to_dict() 
        ...
        op_resp_def = OPERATION_RESULT_DEFINITIONS[op_def["operation_result_data_type"]] #Change resp def structure to match request structure (name is a value, rather than key to obj)
        ...
        if not valid_resp_data_type:
            return OperationResult(data=None, meta={"data_type":"error", "error_message": f"""Mismatch on Operation Result Data Type: {str(result_data)}. 
                                                    Expected Type: {op_def['operation_result_data_type']}, Received Type: {type(result_data)}"""}).to_dict()
        operation.result_data = result_data
        operation.result_data_type = op_def["operation_result_data_type"]
        ...
        #Response Creation
        operation_result = OperationResult(data=operation.result_data, meta={"data_type": operation.result_data_type, 
                                                                             "operation_bid": operation.id,
        ...
        if op_resp_def.get("attach_operations_history"):
            operation_result.meta["operations_history"] = operation_result_util.get_operation_history(user_id=user_id)
        # if op_resp_def.get("attach_unique_value_options"):
        #     operation_result.meta["unique_value_options"] = operation_result_util.get_unique_value_options(user_id=user_id)
        if op_resp_def.get("attach_snapshots_list"):
            operation_result.meta["snapshots_list"] = operation_result_util.get_all_snapshots(user_id=user_id)
