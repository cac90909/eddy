...
class UniversalListService:
    ...
    def get_unique_column_values(self, user_id, data_source, column_name, options_call):
    ...
    def get_unique_json_keys(self, user_id, data_source, options_call):
        list_res = self.universal_repository.get_unique_json_keys(
            user_data_queryset=data_source
        )
        return list_res

    def get_unique_json_key_values(self, user_id, data_source, json_key, options_call):
        list_res = self.universal_repository.get_unique_json_key_values(
            user_data_queryset=data_source, json_key=json_key
        )
        return list_res
    
    def get_unique_json_values(self, user_id, data_source, options_call):
