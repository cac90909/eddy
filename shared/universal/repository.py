...
# --------- Graph Traversal ---------
    ...
    def get_unique_json_keys(self, qs: QuerySet[Universal]) -> Set[str]:
        ...
        qs2, alias = util.create_col_for_all_json_keys(qs)
    ...
    def get_unique_json_key_values(
        ...
        qs3, alias = util.create_col_for_json_key(qs2, key)
    ...
    def aggregate_group_fields(
        ...
        if freq:
            qs, freq_alias = util.create_frequency_col(qs, freq)
            all_group_cols.append(freq_alias)

        # Adapt group columns
        for col in group_cols:
            qs, alias = util.adapt_column_for_processing(qs, col)
